...
def load_partial_commits(repo, branch, start, end, trace=False):
    try:
        commit_page = repo.get_commits(sha=branch, since=start, until=end)
        return (True, commit_page)
    except Exception as e:
        if trace:
            print(f"Fail to load commits of {repo.full_name}/@{branch} due to: {e}")
        return (False, None)


def persist_progress(owner, repo_name, default_branch, commits, base_dir="commit-info", progress_file="progress.csv"):
    ...
    with open(path, 'a') as f:
        f.write(f"{owner}/{repo_name},{default_branch},{commits},{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")


def load_commits(client, owner, repo_name, base_dir="commit-info", trace=False):
    ...
    if not repo:
        ...
        if not Path(csv_file).exists():
            with open(csv_file, 'w') as f:
                f.write("full_name,branch,sha,author_name,author_date,verified\n")
        ...
        with open(jsons_file, 'a') as fh_json:
            with open(csv_file, 'a') as fh_csv:
                for t in daterange(start_date, end_date, slice):
                    ...
                    ok, page = load_partial_commits(repo, default_branch, s, e, trace)
                    if ok:
...
def _write_csv(fh, owner, repo_name, default_branch, raw_data):
    ...
    fh.write(f"{owner}/{repo_name},{default_branch},{sha},{author_name},{author_date},{verified}\n")


# client is the Github instance
# row is a row of Pandas DataFrame
def do_commit_fetch(client, row, base_dir="commit-info", progress_file="progress.csv", trace=False):
    comps = row['full_name'].split('/')
    owner = comps[0]
    name = comps[1]

    t0 = timer()
    default_branch, commits = load_commits(client, owner, name, base_dir, trace)
    persist_progress(owner, name, default_branch, commits, base_dir, progress_file)
    t1 = timer()
    if trace:
...
def grab_commits(repo_csv_file, base_dir="commit-info", progress_file="progress.csv", trace=False):
